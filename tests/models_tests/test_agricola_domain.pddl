...
(:action ag__harvest_collecting_veg
    :parameters (?r - round ?s - stage ?v - vegetable ?i1 - num ?i2 - num ?i3 - num)
    :precondition
    (and
        (hold_round ?r roundend)
        (harvest_phase ?s harvest_init)
...
(:action ag__harvest_feed
    :parameters (?r - round ?s - stage ?wmax - worker ?inow - num ?ifeed - num ?irest - num)
    :precondition
    (and
        (hold_round ?r roundend)
        (harvest_phase ?s harvest_feeding)
...
(:action ag__harvest_breeding_animal
    :parameters (?r - round ?s - stage ?a - animal ?i - num ?i2 - num)
    :precondition
    (and
        (hold_round ?r roundend)
        (harvest_phase ?s harvest_breeding)
...
(:action ag__finish_round_backhome_withchild
    :parameters (?r - round ?maxw - worker ?newmax - worker)
    :precondition
    (and
        (current_round ?r)
        (current_worker noworker)
...
(:action ag__advance_round_normal
    :parameters (?r1 - round ?r2 - round ?act - actiontag)
    :precondition
    (and
...
(:action ag__finish_stage
    :parameters (?s1 - stage ?s2 - stage ?r1 - round ?r2 - round ?act - actiontag)
    :precondition
    (and
...
;; ================================
(:action take_food
    :parameters (?w1 - worker ?w2 - worker ?wmax - worker ?r - round ?i1 - num ?i2 - num)
    :precondition
    (and
        (available_action act_labor)
        (current_worker ?w1)
...
(:action plow_field
    :parameters (?w1 - worker ?w2 - worker ?wmax - worker ?r - round)
    :precondition
    (and
        (available_action act_plow)
        (current_worker ?w1)
...
(:action take_grain
    :parameters (?w1 - worker ?w2 - worker ?wmax - worker ?r - round ?v - vegetable)
    :precondition
    (and
        (available_action act_grain)
        (current_worker ?w1)
...
(:action build_fences
    :parameters (?a - animal ?w1 - worker ?w2 - worker ?wmax - worker ?r - round)
    :precondition
    (and
        (available_action act_fences)
...
(:action collect_animal
    :parameters (?a - animal ?act - animaltag ?w1 - worker ?w2 - worker ?wmax - worker ?r - round)
    :precondition
    (and
...
(:action collect_cook_animal
    :parameters (?a - animal ?act - animaltag ?w1 - worker ?w2 - worker ?wmax - worker ?r - round ?i1 - num ?i2 - num)
    :precondition
    (and
        (available_action ?act)
        (open_action ?act)
...
(:action collect_resource
    :parameters (?w1 - worker ?w2 - worker ?wmax - worker ?r - round ?act - buildtag ?res - resource)
    :precondition
    (and
        (available_action ?act)
        (open_action ?act)
...
(:action build_room
    :parameters (?w1 - worker ?w2 - worker ?wmax - worker ?wnewmax - worker ?r - round ?room - room)
    :precondition
    (and
        (available_action act_build)
...
(:action improve_home
    :parameters (?w1 - worker ?w2 - worker ?wmax - worker ?r - round ?imp - improvement)
    :precondition
    (and
        (available_action act_improve)
...
(:action family_growth
    :parameters (?w1 - worker ?w2 - worker ?wmax - worker ?wnewmax - worker ?r - round ?res - resource ?room - room)
    :precondition
    (and
        (available_action act_family)
