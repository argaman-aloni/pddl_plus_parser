(define (domain driverlog)
  ...
  (:functions
    (time-to-walk ?l1 - location ?l2 - location)
  ...
  (:action LOAD-TRUCK
    ...
    :effect (and (not (at ?obj ?loc)) (in ?obj ?truck) (increase (load ?truck) 1)
      (increase
        (fuel-per-minute ?truck)
        (+ (load ?truck) 1)))
  )
  ...
  (:action UNLOAD-TRUCK
    ...
    :effect (and (not (in ?obj ?truck)) (at ?obj ?loc) (decrease (load ?truck) 1)
