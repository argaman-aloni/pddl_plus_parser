...
from tests.multi_agent_tests.consts import (
    MULTI_AGENT_DATA_DIRECTORY,
    BLOCKS_MULTI_AGENT_DATA_DIRECTORY,
)
...
def test_locate_domains_returns_domain_with_correct_name(
    domain_converter: MultiAgentDomainsConverter,
):
...
def test_locate_domains_returns_all_private_and_public_predicates(
    domain_converter: MultiAgentDomainsConverter,
):
    """Test that the locate_domains method returns a domain with all private and public predicates."""
    domain = domain_converter.locate_domains(add_dummy_actions=True)
    assert "grind-treatment-change" in domain.predicates
    assert "boardsize-successor" in domain.predicates
    assert (
        len(domain.predicates) == 14 + 1
    )  # 14 original predicates + 1 dummy predicate


def test_locate_domains_returns_agents_actions(
    domain_converter: MultiAgentDomainsConverter,
):
...
def test_locate_domains_returns_actions_with_correct_information(
    blocks_domain_converter: MultiAgentDomainsConverter,
):
    """Test that the locate_domains method returns a domain with all agents' actions."""
