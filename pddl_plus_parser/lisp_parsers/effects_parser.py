...
from pddl_plus_parser.lisp_parsers.parsing_utils import (
    parse_untyped_predicate,
    ASSIGNMENT_OPS,
    WHEN_OPERATOR,
    FORALL_OPERATOR,
    NOT_OPERATOR,
...
class EffectsParser:
    ...
    def _parse_result(
        ...
        conditional_effect_ast: List[Union[str, List[str]]],
        domain_constants: Dict[str, PDDLConstant],
        domain_functions: Dict[str, PDDLFunction],
    ) -> Union[Predicate, NumericalExpressionTree]:
        """
        ...
        if conditional_effect_ast[0] == NOT_OPERATOR:
            return parse_untyped_predicate(
                conditional_effect_ast[1], action_signature, domain_constants, False
            )

        if conditional_effect_ast[0] in ASSIGNMENT_OPS:
            return NumericalExpressionTree(
                construct_expression_tree(conditional_effect_ast, domain_functions)
            )

        return parse_untyped_predicate(
            conditional_effect_ast, action_signature, domain_constants, True
        )
        ...
        if conditional_effect_ast[0] == "and":
            for effect in conditional_effect_ast[1:]:
                result = self._parse_result(
                    action_signature, effect, domain_constants, domain_functions
                )
    ...
    def parse_conditional_effect(
        ...
        if len(conditional_effect_ast[1:]) != 2:
            raise SyntaxError(
                f"Conditional effect scheme does not match schema! {conditional_effect_ast}"
            )
        ...
        self.preconditions_parser.parse(
            precondition_root=effect_antecedents.root,
    ...
    def parse_universally_quantified_effect(
        self,
        universal_quantifier_ast: List[Union[str, List[str]]],
        action: Action,
        domain_types: Dict[str, PDDLType],
        domain_predicates: Dict[str, Predicate],
        domain_functions: Dict[str, PDDLFunction],
        ...
        if len(universal_quantifier_ast) != 2:
            raise SyntaxError(
                f"Universal quantifier scheme does not match for action {action.name}!"
            )
        ...
        if len(quantified_object_components) != 3:
            raise SyntaxError(
                f"Quantified object scheme does not match for action {action.name}!"
        ...
        universal_effect = UniversalEffect(
            quantified_parameter=quantified_param, quantified_type=quantified_type
        )
