...
class PreconditionsParser:
    ...
    def parse(
        self,
        precondition_root: Union[Precondition, UniversalPrecondition],
        ...
        for precondition_node in preconditions_ast:
            ...
            if precondition_node[0] == NOT_OPERATOR:
                ...
                if inner_node[0] == EQUALITY_OPERATOR:
                    self.logger.debug(
                        "Adding new lifted objects that should be tested for inequality"
                    )
                    precondition_root.inequality_preconditions.add(
                        (inner_node[1], inner_node[2])
                    )
                    continue
            ...
            if precondition_node[0] == EQUALITY_OPERATOR:
                ...
                self.logger.debug(
                    "Adding new lifted objects that should be tested for equality"
                )
                precondition_root.equality_preconditions.add(
                    (precondition_node[1], precondition_node[2])
                )
                continue
            ...
            if precondition_node[0] == FORALL_OPERATOR:
                ...
                if len(quantified_object_components) != 3:
                    raise SyntaxError(
                        f"Quantified object scheme does not match schema of ( ?param - type )!"
                ...
                if quantified_conditions[0] not in BINARY_OPERATORS:
                    raise SyntaxError(
                        "Quantified conditions must start with binary operator!"
                    )
                ...
                universal_precondition = UniversalPrecondition(
                    quantified_parameter=quantified_param_name,
                ...
                combined_signature = {
                    **action_signature,
                    quantified_param_name: quantified_type,
                }
                self.parse(
                    universal_precondition,
                    quantified_conditions[1:],
                    domain_functions,
                    domain_predicates,
                    domain_types,
                    domain_constants,
