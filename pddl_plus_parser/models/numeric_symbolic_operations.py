...
def extract_atom(
    ...
    if expression.func == Float:
        formatted_expression = (
            format(expression, f".{decimal_digits}f")
            if not round(float(expression), decimal_digits).is_integer()
            else f"{int(expression)}"
        )
...
def _recursive_pow_expression_to_pddl(expression: Pow, symbols_map: dict) -> str:
    ...
    for _ in range(exponent - 1):
        compiled_expression = (
            f"(* {compiled_expression} {symbols_map[expression.base]})"
        )

    return compiled_expression


def _convert_internal_expression_to_pddl(
    expression: Expr,
    operator: str,
    symbols_map: dict,
    decimal_digits: int = DEFAULT_DECIMAL_DIGITS,
    ...
    if expression.is_Atom:
        return extract_atom(
            expression, symbols_map, decimal_digits, should_remove_trailing_zeros
        )

    if isinstance(expression, Pow) and expression.exp == -1:
        pddl_expression = _convert_internal_expression_to_pddl(
            expression.base,
            SYMPY_OP_TO_PDDL_OP[expression.base.func],
            symbols_map,
            decimal_digits,
            should_remove_trailing_zeros,
        )
        return f"(/ 1 {pddl_expression})"
    ...
    for i in range(len(expression.args)):
        comp = _convert_internal_expression_to_pddl(
            expression.args[i],
            SYMPY_OP_TO_PDDL_OP[expression.args[i].func],
            symbols_map,
            decimal_digits,
            should_remove_trailing_zeros,
        )
    ...
    for component in reversed(components):
        if nested_expression:
            nested_expression = (
                f"({operator} {component} {nested_expression})"
                if not is_number_string(components[0])
                else f"({operator} {nested_expression} {component})"
            )
...
def convert_expr_to_pddl(
    expr: Expr,
    symbolic_vars: Dict[str, Symbol],
    decimal_digits: int = DEFAULT_DECIMAL_DIGITS,
    should_remove_trailing_zeros: bool = True,
) -> str:
    ...
    return _convert_internal_expression_to_pddl(
        expr,
        initial_operator,
        {val: key for key, val in symbolic_vars.items()},
...
def transform_expression(
    expression: str, symbols_to_use: Optional[Dict[str, Symbol]] = None
) -> Tuple[str, Dict[str, Symbol]]:
...
def simplify_complex_numeric_expression(
    complex_numeric_expression: str, decimal_digits: int = DEFAULT_DECIMAL_DIGITS
) -> str:
    ...
    return convert_expr_to_pddl(
        simplified_expression, symbolic_vars, decimal_digits=decimal_digits
    )


def simplify_equality(
    equation: str, decimal_digits=DEFAULT_DECIMAL_DIGITS
) -> Optional[str]:
    """
    ...
    transformed_right_expr, symbolic_vars = transform_expression(
        right_expr, symbolic_vars
    )
    ...
    pddl_left_side = convert_expr_to_pddl(
        simplified_equation.lhs, symbolic_vars, decimal_digits=decimal_digits
    )
    pddl_right_side = convert_expr_to_pddl(
        simplified_equation.rhs,
        symbolic_vars,
        decimal_digits=decimal_digits,
...
def simplify_inequality(
    complex_numeric_expression: str,
    inequality_operator: str,
    ...
    :return: the simplified expression in PDDL format.
    """
    left_side_expression, right_side_expression = complex_numeric_expression[
        1:-1
    ].split(inequality_operator)
    transformed_left_str, symbolic_vars = transform_expression(left_side_expression)
    transformed_right_str, symbolic_vars = transform_expression(
        right_side_expression, symbolic_vars
    )
    ...
    for assumption_str in assumptions:
        ...
        assumption_expression, symbolic_vars = transform_expression(
            assumption_str, symbolic_vars
        )
    ...
    pddl_left_side = convert_expr_to_pddl(
        expand(left_expr), symbolic_vars, decimal_digits=decimal_digits
    )
    pddl_right_side = convert_expr_to_pddl(
        expand(right_expr),
        symbolic_vars,
