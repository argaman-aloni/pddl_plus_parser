...
class MultiAgentProblemsConverter:
    ...
    def combine_problems(self, combined_domain_path: Path) -> Problem:
        ...
        combined_domain = DomainParser(
            domain_path=combined_domain_path, partial_parsing=False
        ).parse_domain()
        combined_problem = Problem(domain=combined_domain)
        for problem_file_path in self.problems_directory_path.glob(
            f"{self.problem_file_prefix}-*.pddl"
        ):
            agent_problem = ProblemParser(
                problem_path=problem_file_path, domain=combined_domain
            ).parse_problem()
            combined_problem.name = agent_problem.name
            combined_problem.objects.update(agent_problem.objects)
            combined_problem.initial_state_fluents.update(
                agent_problem.initial_state_fluents
            )
            for (
                predicate,
                grounded_predicates,
            ) in agent_problem.initial_state_predicates.items():
                combined_state_predicates = [
                    p.untyped_representation
                    for p in combined_problem.initial_state_predicates[predicate]
                ]
                for grounded_predicate in grounded_predicates:
                    if (
                        grounded_predicate.untyped_representation
                        in combined_state_predicates
                    ):
                        continue

                    combined_problem.initial_state_predicates[predicate].add(
                        grounded_predicate
                    )

            combined_problem.goal_state_predicates.extend(
                agent_problem.goal_state_predicates
            )
            combined_problem.goal_state_predicates = list(
                set(combined_problem.goal_state_predicates)
            )
    ...
    def export_combined_problem(self, combined_domain_path: Path) -> None:
        ...
        ProblemExporter().export_problem(
            combined_problem, self.problems_directory_path / "combined_problem.pddl"
        )
